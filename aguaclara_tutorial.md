...
I'm Dylan, and I'm from northern New Jersey.

```Note to self that putting things like this makes it look like code```

```python
#That's because this how you get python code.  Make note that you have to bookend the code like this.
```
...
### This is the required text with Header 3.
...
## Things I hope to do this semester
* Focus more on grades, and balancing everything that I do
* Stop pushing administrative tasks aside until the last minute
* Start paying more attention to my emails
...
Unformatted:
![Berkeley](https://github.com/dylanvucornell/Personalreal/blob/master/1200px-Berkeley_Heights_NJ_shopping_center_in_town.jpg?raw=true)

Using a Relative File Path:
![Berkely](/1200px-Berkeley_Heights_NJ_shopping_center_in_town.jpg)

Formatted:
<img src="https://github.com/dylanvucornell/Personalreal/blob/master/1200px-Berkeley_Heights_NJ_shopping_center_in_town.jpg?raw=true" height = 200 width = 50>
...
Chemical Engineering the branch of engineering concerned with the design and operation of industrial chemical plants.  Please see the [website](https://www.cheme.cornell.edu/) for more.
...
Favorites:
| Animal | Food | Book | Campus Location |
|:---------- |:----------: | :----------: | :-------- |
| Monkeys   | Sushi  | reading?    | Becker House    |
...
for i in range(zacslist):
  arraywithunits[i] = myarray[i] * u.m
  print(myarrayunits[i])

x = numpy.arange(1, 6, 1)
print(x)
myarray = numpy.array([x, x, x, x, x])
print(myarray)
numpyp.shape(myarray)
midCol = myarray[:,2]

for i in range(5):
  midColUnits[i] = midCol[i] * u.L
  print(midColUnits[i])

midrow = myarray[2,:]
midRowUnits = midRow * u.L
```
...
def myfunc(T, n, r):
  ...
  n = n.to(u.kg / (u.m * u.s))
  r = r.to(u.m)
  D = ((kB * T) / (6 * m.pi * n * r)).to_base_units()
  return D

myfunc(10*u.degK, 10*u.kg / (u.m * u.s), 5*u.m)
```
...
array = (np.arange(0, 200, 1)) * u.degC
r = 0.2 * u.m
Q  = 2 * (u.m**3 / u.s)
nArray = pc.viscosity_kinematic(array)
rArray = pc.re_pipe(Q, r, nArray)
plt.plot(array.to(u.degK), rArray)
...
<!--- Fill you answer here. --->
Hey Dylan, keep being awesome!!!  -- Liz

Hi Dylan, You Thick! -- Alison
